...
if (packages.length === 0) {
  console.error("No packages provided. Usage: bun ./prepare-publish.ts ./packages/cli/hydra ./packages/cli/sisyphus");
  process.exit(1);
}

console.info(`Removing dev dependencies from ${packages.length} package${packages.length > 1 ? "s" : ""}`);
