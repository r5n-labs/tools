...
const LOG_CONFIGS = [
  { color: colors.dim, console: console.log, label: "DEBUG", level: LogLevel.DEBUG, method: "debug" },
  { color: colors.cyan, console: console.info, label: "INFO ", level: LogLevel.INFO, method: "info" },
  { color: colors.yellow, console: console.warn, label: "WARN ", level: LogLevel.WARN, method: "warn" },
  { color: colors.red, console: console.error, label: "ERROR", level: LogLevel.ERROR, method: "error" },
] as const;
...
export const createLogger = ({
  ...
  logger.child = (childPrefix: string) => createLogger({ level, prefix: `${prefix}:${childPrefix}` });
